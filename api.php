...
switch ($action) {
    ...
    case 'leads_dedupe_x9': {
        // تنظيف لمرة واحدة: دمج سجلات المهتمين المكررة بنفس الجوال — يُحذف بعد التنفيذ
        if (($_GET['k'] ?? '') !== 'semak-diag-8891') { echo json_encode(['success'=>false]); break; }
        $res = $conn->query("SELECT id, name, phone, interest, unit, status, notes, summary FROM leads ORDER BY id DESC");
        $groups = [];
        while ($res && ($r = $res->fetch_assoc())) {
            $digits = preg_replace('/\D/', '', $r['phone'] ?? '');
            $key = $digits !== '' ? substr($digits, -9) : 'id:' . $r['id'];
            $groups[$key][] = $r; // الأحدث أولاً بحكم الترتيب
        }
        $statusRank = ['تم البيع' => 4, 'مهتم' => 3, 'جديد' => 2, 'مرفوض' => 1];
        $merged = 0; $deleted = 0;
        foreach ($groups as $arr) {
            if (count($arr) < 2) continue;
            $keeper = $arr[0];
            // أفضل اسم: أطول اسم حقيقي (ليس "عميل واتساب" ولا حرفاً واحداً)
            $bestName = $keeper['name'];
            foreach ($arr as $x) {
                $n = trim($x['name'] ?? '');
                if ($n !== '' && $n !== 'عميل واتساب' && mb_strlen($n) > mb_strlen($bestName === 'عميل واتساب' ? '' : $bestName)) $bestName = $n;
            }
            // أعلى حالة تقدماً
            $bestStatus = $keeper['status'] ?: 'جديد';
            foreach ($arr as $x) { if (($statusRank[$x['status'] ?? ''] ?? 0) > ($statusRank[$bestStatus] ?? 0)) $bestStatus = $x['status']; }
            // دمج الاهتمامات الفريدة
            $ints = [];
            foreach ($arr as $x) { $v = trim($x['interest'] ?? ''); if ($v && $v !== 'استفسار واتساب' && !in_array($v, $ints)) $ints[] = $v; }
            $interest = $ints ? implode('، ', $ints) : ($keeper['interest'] ?: 'استفسار واتساب');
            // أفضل وحدة: أول قيمة محددة
            $unit = '';
            foreach ($arr as $x) { $v = trim($x['unit'] ?? ''); if ($v && $v !== 'غير محدد' && $v !== 'استفسار واتساب') { $unit = $v; break; } }
            if ($unit === '') $unit = $keeper['unit'] ?? '';
            // دمج الملاحظات: أسطر فريدة، الأحدث أولاً، سقف 8000 حرف
            $lines = [];
            foreach ($arr as $x) {
                foreach (preg_split('/\n+/', (string)($x['notes'] ?? '')) as $ln) {
                    $ln = trim($ln);
                    if ($ln !== '' && !in_array($ln, $lines)) $lines[] = $ln;
                }
            }
            $notes = mb_substr(implode("\n", $lines), 0, 8000);
            // أحدث ملخص متوفر
            $summary = '';
            foreach ($arr as $x) { if (trim($x['summary'] ?? '') !== '') { $summary = $x['summary']; break; } }

            $kid = (int)$keeper['id'];
            $conn->query("UPDATE leads SET
                name='"     . $conn->real_escape_string($bestName)   . "',
                status='"   . $conn->real_escape_string($bestStatus) . "',
                interest='" . $conn->real_escape_string($interest)   . "',
                unit='"     . $conn->real_escape_string($unit)       . "',
                notes='"    . $conn->real_escape_string($notes)      . "',
                summary="   . ($summary !== '' ? "'" . $conn->real_escape_string($summary) . "'" : "summary") . "
                WHERE id=$kid");
            $ids = array_map(fn($x) => (int)$x['id'], array_slice($arr, 1));
            if ($ids) { $conn->query("DELETE FROM leads WHERE id IN (" . implode(',', $ids) . ")"); $deleted += count($ids); }
            $merged++;
        }
        echo json_encode(['success'=>true, 'groups_merged'=>$merged, 'rows_deleted'=>$deleted], JSON_UNESCAPED_UNICODE);
        break;
    }

    case 'leads_summarize_x9': {
        // توليد ملخصات لمرة واحدة للعملاء الحاليين من ملاحظاتهم — يُحذف بعد التنفيذ
        if (($_GET['k'] ?? '') !== 'semak-diag-8891') { echo json_encode(['success'=>false]); break; }
        $res = $conn->query("SELECT id, name, interest, unit, status, notes FROM leads
                             WHERE (summary IS NULL OR summary='') AND notes IS NOT NULL AND notes != ''
                             ORDER BY id DESC LIMIT 6");
        $done = 0; $fail = 0; $remaining = 0;
        $rows = [];
        while ($res && ($r = $res->fetch_assoc())) $rows[] = $r;
        foreach ($rows as $r) {
            $notes = mb_substr((string)$r['notes'], 0, 3500);
            $prompt = "أنت مساعد مبيعات عقاري. لخّص وضع هذا العميل المهتم في سطر أو سطرين بالعربية، "
                    . "يشمل: من هو، غرضه (سكن/استثمار)، ميزانيته إن ذُكرت، الوحدة المفضلة، آخر تطور، والخطوة التالية المقترحة لفريق المبيعات. "
                    . "أرجع الملخص فقط بلا مقدمات.\n\n"
                    . "الاسم: {$r['name']}\nالاهتمام: {$r['interest']}\nالوحدة: {$r['unit']}\nالحالة: {$r['status']}\nالملاحظات المتراكمة (الأحدث أولاً):\n$notes";
            ...
            $txt = trim($txt);
            if ($txt !== '') {
                $conn->query("UPDATE leads SET summary='" . $conn->real_escape_string(mb_substr($txt, 0, 590)) . "' WHERE id=" . (int)$r['id']);
                $done++;
            } else $fail++;
        }
        $rc = $conn->query("SELECT COUNT(*) c FROM leads WHERE (summary IS NULL OR summary='') AND notes IS NOT NULL AND notes != ''");
        if ($rc && ($rr = $rc->fetch_assoc())) $remaining = (int)$rr['c'];
        echo json_encode(['success'=>true, 'summarized'=>$done, 'failed'=>$fail, 'remaining'=>$remaining], JSON_UNESCAPED_UNICODE);
        break;
    }
