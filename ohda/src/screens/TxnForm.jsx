...
export default function TxnForm({ id, q }) {
    ...
    const onFile = async e => {
        ...
        if (!r.success) { setStage(''); setErr(r.message || 'تعذّر رفع الملف'); return; }
        setFile({ id: r.id, url: r.url, mime: r.mime });
        if (!flags.ai || !out) { setStage(''); return; }
        setStage('reading');
        const s = await call('scan', { body: { file_id: r.id } });
        setStage('');
        if (!s.success) { setErr((s.message || 'تعذّرت القراءة') + ' — أكمل البيانات يدوياً'); return; }
        const x = s.data;
        setScan(x);
    ...
    const save = async (force, again) => {
        setErr('');
        if (!(Number(f.amount) > 0)) { setErr('اكتب المبلغ'); amt.current && amt.current.focus(); return; }
        setBusy(true);
        ...
        if (!r.success) { setErr(r.message || 'تعذّر الحفظ'); return; }
        reloadFunds();
        toast(f.id ? 'حُفظت التعديلات' : (out ? 'سُجّل المصروف' : 'سُجّل الاستلام'));
        if (again) { setF({ ...blank({ type: f.type, fund: f.fund_id }, funds), d: f.d }); setFile(null); setScan(null); window.scrollTo(0, 0); return; }
        back('/txns');
    };

    const remove = async () => {
        if (!window.confirm('نقل الحركة إلى السلّة؟ تُستعاد منها متى شئت.')) return;
        const r = await call('txn_delete', { body: { id: f.id } });
        if (r.success) { reloadFunds(); toast('نُقلت إلى السلّة'); back('/txns'); }
    };
    ...
    return (
        <div className="pt-2 lg:pt-0 lg:grid lg:grid-cols-5 lg:gap-8 lg:items-start">
            ...
            <div className="lg:col-span-2 lg:sticky lg:top-6 space-y-3 mb-5">
                {!f.id ? (
                    <Seg value={f.type} onChange={v => set('type', v)} options={[{ v: 'out', t: 'مصروف' }, { v: 'in', t: 'استلام مبلغ' }]} />
                ) : null}
                ...
                {file ? (
                    <div className="relative rounded-2xl overflow-hidden border border-paper-2 bg-paper-card">
                        {file.mime === 'application/pdf' ? (
                            <a href={file.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-5 text-brand-700 font-semibold">
                                <FileText size={28} />مستند PDF — اضغط للعرض
                            </a>
                        ) : (
                            <a href={file.url} target="_blank" rel="noreferrer">
                                <img src={file.url} alt="الإيصال" className="w-full max-h-[220px] lg:max-h-[520px] object-contain bg-paper-2" />
                            </a>
                        )}
                        <div className="flex gap-2 p-2 border-t border-paper-2">
                            <Btn kind="ghost" className="flex-1 !h-9 text-[13px]" onClick={() => cam.current.click()}><Camera size={15} />إعادة التصوير</Btn>
                            <Btn kind="ghost" className="!h-9 text-[13px]" onClick={() => { setFile(null); setScan(null); }}><Trash2 size={15} />إزالة</Btn>
                        </div>
                        {stage ? (
                            <div className="absolute inset-0 bg-paper-card/85 backdrop-blur-sm flex flex-col items-center justify-center gap-2">
                                ...
                                <span className="text-[13px] font-semibold text-ink-2">{stage === 'uploading' ? 'يُرفع الملف…' : 'يقرأ الإيصال…'}</span>
                            </div>
                        ) : null}
                    </div>
                ) : (
                    <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => cam.current.click()} disabled={!!stage}
                            ...
                            {stage === 'uploading' ? 'يُرفع…' : 'صوّر الإيصال'}
                        </button>
                ...
                {out && flags.ai && !file ? (
                    <p className="flex items-center gap-1.5 text-[12px] text-ink-3 px-1">
                        <Sparkles size={13} className="text-amber" />يقرأ التطبيق الجهة والتاريخ والمبلغ والضريبة من الإيصال
                    </p>
                ) : null}
                ...
                {scan ? (
                    <div className={'rounded-2xl border p-3 text-[12.5px] ' + (scan.confidence === 'low' ? 'bg-amber-50 border-amber-100' : 'bg-brand-50 border-brand-100')}>
                        <div className="flex items-center gap-1.5 font-bold text-brand-800">
                            <Sparkles size={14} />قُرئ الإيصال
                            <span className="ms-auto text-[11px] font-semibold opacity-70">
                                {scan.confidence === 'high' ? 'قراءة واضحة' : scan.confidence === 'medium' ? 'راجع الأرقام' : 'قراءة ضعيفة — راجعها'}
                            </span>
                        </div>
                        ...
                        {scan.items && scan.items.length ? (
                            <>
                                <button onClick={() => setShowItems(v => !v)} className="mt-2 flex items-center gap-1 font-semibold text-brand-700">
                                    {scan.items.length} بنود <ChevronDown size={14} className={showItems ? 'rotate-180' : ''} />
                                </button>
                                {showItems ? (
            ...
            <div className="lg:col-span-3 space-y-4">
                <div className="bg-paper-card rounded-2xl border border-paper-2 p-4">
                    <label className="block text-[12px] font-semibold text-ink-2 mb-1">المبلغ</label>
                    <div className="flex items-baseline gap-2" dir="ltr">
                        ...
                        <span className="text-ink-3 font-semibold">ر.س</span>
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                    <Field label="التاريخ" hint={f.d !== today() ? fullDate(f.d) : null}>
                        <input type="date" className={inputCls} value={f.d} onChange={e => set('d', e.target.value)} />
                    </Field>
                    {out ? (
                        <Field label="الضريبة (١٥٪)">
                            <div className="relative">
                                <input inputMode="decimal" className={inputCls + ' pl-10'} dir="ltr" value={f.vat} placeholder="0"
                                    onChange={e => set('vat', e.target.value.replace(/[^\d.]/g, ''))} />
                                <button type="button" title={'احسب من المبلغ: ' + money(vatGuess, 2)} onClick={() => set('vat', String(vatGuess))}
                                    className="absolute left-1 top-1 w-9 h-9 rounded-lg text-ink-3 hover:bg-paper-2 flex items-center justify-center">
                                    <Calculator size={16} />
                                </button>
                            </div>
                        </Field>
                        ...
                        <Field label="المرجع">
                            <input className={inputCls} value={f.ref} onChange={e => set('ref', e.target.value)} placeholder="رقم التحويل" />
                        </Field>
                    )}
                </div>

                <Field label={out ? 'الجهة' : 'المصدر'}>
                    <input className={inputCls} list="oh-vendors" value={f.vendor} onChange={e => pickVendor(e.target.value)}
                        placeholder={out ? 'المتجر أو المحطّة أو المدرسة' : 'من سلّمك المبلغ'} />
                    <datalist id="oh-vendors">{vendors.map(v => <option key={v.vendor} value={v.vendor} />)}</datalist>
                </Field>

                {out ? (
                    <div>
                        <span className="block text-[12px] font-semibold text-ink-2 mb-1.5">التصنيف</span>
                ...
                {openFunds.length ? (
                    <div>
                        <span className="block text-[12px] font-semibold text-ink-2 mb-1.5">{out ? 'من عهدة' : 'إلى عهدة'}</span>
                        <div className="flex flex-wrap gap-1.5">
                            {openFunds.map(x => (
                ...
                <div>
                    <span className="block text-[12px] font-semibold text-ink-2 mb-1.5">طريقة الدفع</span>
                    <Seg value={f.method} onChange={v => set('method', v)} options={Object.entries(METHODS).map(([v, t]) => ({ v, t }))} />
                </div>

                {out ? (
                    <Field label="رقم الفاتورة أو المرجع">
                ...
                <div className="flex gap-2 pt-1 sticky bottom-0 lg:bottom-4 bg-paper/90 backdrop-blur py-2 -mx-1 px-1 z-10"
                    ...
                    <Btn className="flex-1 !h-12 text-[15px]" busy={busy} onClick={() => save(false, false)}><Check size={18} />{f.id ? 'حفظ التعديلات' : 'حفظ'}</Btn>
                    {!f.id ? <Btn kind="soft" className="!h-12" disabled={busy} onClick={() => save(false, true)}>حفظ وإضافة آخر</Btn> : null}
                    {f.id ? <Btn kind="danger" className="!h-12 !px-3" onClick={remove} aria-label="حذف"><Trash2 size={18} /></Btn> : null}
                </div>
            </div>

            <Sheet open={!!dup} onClose={() => setDup(null)} title="يبدو أنها مسجّلة من قبل">
                {dup ? (
                    <div className="space-y-4">
                        <p className="text-[14px] text-ink-2 leading-7">
                            في <b>{fullDate(dup.d)}</b> سُجّل <b>{dup.vendor || 'مصروف'}</b> بمبلغ <Money v={dup.amount} className="font-bold" /> — بنفس اليوم والجهة والمبلغ.
                        </p>
                        <div className="flex gap-2">
                            <Btn kind="line" className="flex-1" onClick={() => setDup(null)}>لا، ألغِ</Btn>
                            <Btn className="flex-1" onClick={() => { const a = dup.again; setDup(null); save(true, a); }}>سجّلها مرّة ثانية</Btn>
                        </div>
                        <a href={'#/txn/' + dup.id} className="block text-center text-[13px] font-semibold text-brand">افتح المسجَّلة</a>
                    </div>
                ) : null}
            </Sheet>
        </div>
