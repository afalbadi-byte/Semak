            ...
            <div className="grid lg:grid-cols-3 gap-3">
                <a href={href('/txns', { type: 'out', ...month })} className="bg-paper-card rounded-2xl border border-paper-2 p-4 lg:col-span-2">
                    <div className="flex justify-between text-[12px] text-ink-3 font-semibold">
                        <span>مصروف هذا الشهر من الميزانية</span><span>اليوم {day} من {days}</span>
                    </div>
                    <div className="flex items-baseline gap-2 mt-1">
                        <Money v={usedBudgeted} className="text-[28px] font-bold" />
                        {totBudget ? <span className="text-ink-3 text-[14px]">من <Money v={totBudget} /></span> : null}
                    </div>
                    {totBudget ? (
                        <div className="relative mt-3">
                            <Progress used={usedBudgeted} total={totBudget} />
                            {/* علامة موضع اليوم من الشهر: ما قبلها صرفٌ في موعده */}
                            <span className="absolute -top-1 h-4 w-0.5 bg-ink/40 rounded" style={{ right: (day / days) * 100 + '%' }} />
                        </div>
                    ) : <p className="text-[12.5px] text-ink-3 mt-2">حدّد ميزانية لتصنيفاتك ليظهر هنا تقدّمك وتنبيهك قبل التجاوز.</p>}
                </a>
                <a href={href('/txns', { type: 'out', ...month })} className="bg-paper-card rounded-2xl border border-paper-2 p-4">
                    <div className="text-[12px] text-ink-3 font-semibold">كل مصروف الشهر</div>
                    <Money v={totUsed} className="block text-[28px] font-bold mt-1" />
                    <div className="text-[12px] text-ink-3 mt-1">منه خارج الميزانيات <Money v={totUsed - usedBudgeted} cur={false} /></div>
                </a>
            </div>

            <div className="flex items-center justify-between">
                <h2 className="text-[13px] font-bold text-ink-2 px-1">التصنيفات</h2>
                <Btn kind="soft" onClick={() => setEdit({ name: '', icon: 'tag', color: PALETTE[cats.length % PALETTE.length], budget: 0 })}><Plus size={17} />تصنيف</Btn>
            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <a href={href('/txns', { cat: c.id, ...month })} className="flex-1 min-w-0">
                                    ...
                                    <div className="text-[12px] text-ink-3">
                                        <Money v={u} cur={false} />{c.budget ? <> من <Money v={c.budget} cur={false} /></> : ' · بلا سقف'}
                                    </div>
                                </a>
                                <button onClick={() => setEdit(c)} className="w-9 h-9 rounded-xl hover:bg-paper-2 flex items-center justify-center text-ink-3" aria-label="تعديل"><Pencil size={15} /></button>
                            </div>
...
function CatSheet({ c, onClose, onSaved }) {
    ...
    const save = async del => {
        if (del && !window.confirm('حذف التصنيف؟ تبقى حركاته مسجّلة بلا تصنيف.')) return;
        setBusy(true);
        ...
        await onSaved(); toast(del ? 'حُذف التصنيف' : 'حُفظ'); onClose();
    };
    return (
        <Sheet open={!!c} onClose={onClose} title={v.id ? 'تعديل التصنيف' : 'تصنيف جديد'}>
            <div className="space-y-4">
                <Field label="الاسم"><input className={inputCls} value={v.name} onChange={e => setV({ ...v, name: e.target.value })} /></Field>
                <Field label="الميزانية الشهرية" hint="اتركها صفراً إن لم ترد سقفاً">
                ...
                <div>
                    <span className="block text-[12px] font-semibold text-ink-2 mb-1.5">الأيقونة</span>
                ...
                <div className="flex gap-2">
                    <Btn className="flex-1" busy={busy} onClick={() => save(false)}>حفظ</Btn>
                    {v.id ? <Btn kind="danger" onClick={() => save(true)}>حذف</Btn> : null}
                </div>
            </div>
        </Sheet>
    );
