...
export default function AyahPicker({ label, value, onChange, min }) {
    ...
    // آيات السورة كلّها بأوائل كلماتها: تظهر في قائمة الاختيار نفسها
        ...
        <div className="space-y-1">
            ...
            <div className="grid grid-cols-[112px_1fr] gap-1.5">
                ...
                <select className={sel + ' tabular-nums'} value={a} onChange={e => onChange(key(s, +e.target.value))} aria-label="رقم الآية">
                    {Array.from({ length: AYAT[s - 1] }, (_, i) => i + 1).map(n => (lo && s === lo.s && n < lo.a ? null : <option key={n} value={n}>{n}{ch && ch[n - 1] ? ' · ' + ch[n - 1].t : ''}</option>))}
                </select>
            </div>
