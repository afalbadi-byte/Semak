...
// children: أقسامٌ إضافية في النافذة (مثل «استمع من هذه الآية»)، noMarks: لمسُ رقم آيةٍ لا كلمة
export default function WordActions({ word, mark, onOp, onClose, busy, children, noMarks }) {
    if (!word) return null;
    ...
    return (
        <div className="fixed inset-x-0 bottom-0 z-40 px-3" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 12px)' }}>
            <div className="max-w-lg mx-auto bg-paper-card rounded-3xl border border-paper-2 shadow-2xl p-4 al-rise max-h-[85vh] overflow-y-auto">
