            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-5 md:p-6 border-b border-slate-100 bg-slate-50/50 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-[#1a365d]">دفتر اليومية</h3>
                            <p className="text-slate-400 text-xs font-bold">كل القيود في المحرّك المستقل — انقر القيد لعرض تفاصيله</p>
                        </div>
                    </div>
                ...
                <div className="px-5 md:px-6 pt-4 flex justify-end">
                    <button onClick={load} title="تحديث"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[13px] font-bold border border-slate-200 text-slate-500 hover:border-[#c5a059] transition">
                ...
                <div className="p-4 md:p-5">
                    {loading ? (
                        ...
                        <div className="text-center py-16 text-slate-300 font-bold">لا توجد قيود مطابقة</div>
                    ) : (
                        <div className="overflow-x-auto custom-scrollbar">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {filtered.map(e => (
                                        <tr key={e.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition">
                                            <td className="py-2.5 px-3">
                                                <EntityLink to={`entry/${e.id}`} icon={Hash}>{e.entry_no || `#${e.id}`}</EntityLink>
                                            </td>
                                            <td className="py-2.5 px-3 text-slate-500 font-bold whitespace-nowrap" dir="ltr">{e.date}</td>
                                            <td className="py-2.5 px-3 text-slate-600 font-bold max-w-xs truncate">{e.description || '—'}</td>
                                            <td className="py-2.5 px-3 text-slate-500 font-bold text-[12px]">{REF_LABELS[e.ref_type] || e.ref_type || '—'}</td>
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    {entry.description && <p className="mt-1.5 text-sm font-bold text-slate-600">{entry.description}</p>}
                                </div>
                            </div>
                            {/* التوازن */}
                            <div className={`text-left rounded-2xl px-5 py-3 border ${balanced ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-200'}`}>
                                <div className="text-[11px] font-bold text-slate-400 mb-0.5 text-left">إجمالي القيد</div>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {lines.length === 0 ? (
                                        ...
                                        return (
                                            <tr key={l.id || i} className="border-b border-slate-50 hover:bg-slate-50/60 transition">
                                                <td className="py-2.5 px-3">
                                                    <span className="text-slate-400 font-bold text-[12px] ml-1" dir="ltr">{l.account_code}</span>
                                                    <span className="text-slate-700 font-bold">{l.account_name}</span>
                                                </td>
                                                <td className="py-2.5 px-3">
                                                    {pid
                                                ...
                                                <td className="py-2.5 px-3 text-slate-500 font-bold">{l.description || '—'}</td>
