...
export default function OwnersManage({ showToast }) {
    ...
    const [newOwner, setNewOwner] = useState({ name: "", phone: "", email: "", national_id: "", project_id: "", unit_code: "" });
    ...
    const handleAddOwner = async (e) => {
        ...
        try {
            ...
            if(data.success) {
                ...
                setNewOwner({ name: "", phone: "", email: "", national_id: "", project_id: projectsData[0]?.id || "", unit_code: "" });
                loadData();
            } else {
            ...
            {showAddOwner && !editingOwner && (
                <form onSubmit={handleAddOwner} className="bg-emerald-50 p-6 md:p-8 rounded-2xl mb-8 border border-emerald-100 shadow-inner animate-fadeIn">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                        ...
                        <div><label className="text-xs font-bold text-slate-600 dark:text-brand-300 block mb-2">رقم الهوية أو الإقامة (اختياري)</label><input value={newOwner.national_id} onChange={(e) => setNewOwner({...newOwner, national_id: e.target.value.replace(/D/g, '')})} dir="ltr" maxLength={10} className="w-full p-3 rounded-xl border border-slate-200 dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 dark:placeholder-brand-500 outline-none focus:border-emerald-500 text-left" placeholder="1XXXXXXXXX" /></div>
                    </div>
            ...
            {editingOwner && (
                <form onSubmit={handleUpdateOwner} className="bg-blue-50 p-6 md:p-8 rounded-2xl mb-8 border border-blue-200 shadow-inner animate-fadeIn relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                        ...
                        <div><label className="text-xs font-bold text-slate-600 dark:text-brand-300 block mb-2">رقم الهوية أو الإقامة</label><input value={editingOwner.national_id || ''} onChange={(e) => setEditingOwner({...editingOwner, national_id: e.target.value.replace(/D/g, '')})} dir="ltr" maxLength={10} className="w-full p-3 rounded-xl border border-slate-200 dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 outline-none focus:border-blue-500 text-left" placeholder="1XXXXXXXXX" /></div>
                    </div>
                            ...
                            {ownersList.map(o => (
                                <tr key={o.id} className="hover:bg-emerald-50/50 dark:hover:bg-brand-800 transition group">
                                    <td className="p-4">
                                        ...
                                        {o.national_id && <><br/><span className="text-[11px] text-slate-400 font-mono" dir="ltr">{o.national_id}</span></>}
                                    </td>
                                    <td className="p-4">
