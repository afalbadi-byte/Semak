...
function StatusBadge({ value }) {
    ...
    if (v === 'active' || v === 'نشط' || v === '1' || v === 'approved' || v === 'معتمد' || v === 'completed' || v === 'مكتمل') {
        return (
            <span className="inline-flex items-center gap-1 bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30 text-xs font-bold px-2 py-0.5 rounded-full">
                {value}
            </span>
        );
    }
    if (v === 'inactive' || v === 'غير نشط' || v === '0' || v === 'false' || v === 'cancelled' || v === 'ملغي') {
        return (
            <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-brand-800 text-slate-500 dark:text-brand-400 border border-slate-200 dark:border-brand-700 text-xs font-bold px-2 py-0.5 rounded-full">
                {value}
            </span>
        );
    }
    if (v === 'draft' || v === 'مسودة' || v === 'pending' || v === 'معلّق') {
        return (
            <span className="inline-flex items-center gap-1 bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30 text-xs font-bold px-2 py-0.5 rounded-full">
                {value}
            </span>
        );
    }
    if (v === 'refunded' || v === 'مسترد' || v === 'returned' || v === 'مرتجع') {
        return (
            <span className="inline-flex items-center gap-1 bg-purple-100 dark:bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/30 text-xs font-bold px-2 py-0.5 rounded-full">
                {value}
            </span>
        );
    }
    return (
        <span className="inline-flex items-center gap-1 bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-500/30 text-xs font-bold px-2 py-0.5 rounded-full">
            {value}
        </span>
    );
}
...
function ErrorState({ error, httpCode }) {
    return (
        <div className="flex flex-col items-center justify-center py-16 text-red-600">
            ...
            <p className="text-sm mt-1 bg-red-50 dark:bg-rose-500/10 border border-red-200 dark:border-rose-500/30 rounded-xl px-4 py-2 font-mono dark:text-rose-300">{error}</p>
            {httpCode !== null && httpCode !== undefined && (
                <p className="text-xs mt-2 text-slate-500 dark:text-brand-400 font-mono">
                    رمز الاستجابة: <span className="font-bold text-red-700 dark:text-rose-400">HTTP {httpCode}</span>
                </p>
            )}
        </div>
    );
        ...
        <div className="bg-white dark:bg-brand-900 rounded-2xl border border-slate-100 dark:border-brand-700 shadow overflow-hidden">
            <div className="overflow-x-auto">
                <table className="w-full text-right text-sm">
                    <thead className="bg-brand-800 dark:bg-brand-900 text-white text-xs">
                        <tr>
                            {columns.map(col => (
...
function SummaryChips({ count, amountField, amountTotal }) {
    return (
        <div className="flex items-center gap-2 flex-wrap">
            <span className="inline-flex items-center gap-1.5 bg-brand-800/5 dark:bg-brand-800/30 border border-brand-800/20 dark:border-brand-700 text-brand-800 dark:text-brand-300 text-xs font-bold px-3 py-1.5 rounded-full">
                <Hash size={12} className="text-gold-500" />
                {count} سجل
            </span>
            {amountField && (
                <span className="inline-flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold px-3 py-1.5 rounded-full">
                    <DollarSign size={12} />
    ...
    return (
        <div className="relative flex-1 max-w-xs">
            ...
            <input
                ...
                className="w-full bg-slate-50 dark:bg-brand-900 border border-slate-200 dark:border-brand-700 pr-8 pl-3 py-2 rounded-xl text-sm outline-none focus:border-[#c5a059] dark:text-brand-50 dark:placeholder-brand-500"
            />
        </div>
    );
}
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                    {data && <span className="text-sm text-slate-500 dark:text-brand-400">{meta.total || rows.length} {unitLabel}</span>}
