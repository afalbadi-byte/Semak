...
import { Calculator, Save, RefreshCw, Plus, Trash2, Users, FileSpreadsheet, Presentation, LayoutDashboard, Landmark, Briefcase, FileText, DownloadCloud } from 'lucide-react';
...
export default function FeasibilityCalc({ showToast }) {
    ...
    // =======================================================
    // 🔥 التصدير للـ PDF (بالتصميم المطابق للصورة 100%)
    // =======================================================
    const exportToPDF = (type) => {
        
        // تجهيز بيانات المستثمر لو كان الاختيار "مستثمر محدد"
        ...
        // إنشاء جدول المستثمرين للملحق التفصيلي
        ...
        // بناء محتوى الـ HTML المشترك (الهيدر والفوتر والستايل)
        const htmlTemplate = `
            <!DOCTYPE html>
            ...
            <head>
                ...
                <style>
                    * { box-sizing: border-box; }
                    ...
                    /* الأشرطة العلوية والسفلية حسب الصورة */
                                ...
                                <table style="width: 100%; text-align: right; border-collapse: collapse; font-size: 13px;">
                                    <tbody style="font-weight: bold;">
                                        <tr style="border-bottom: 1px solid #e2e8f0;">
                                            <td style="padding: 12px; background-color: #f8fafc; color: #475569; width: 65%;">رأس المال التأسيسي (الأرض + التأسيس والرخص)</td>
                                            <td style="padding: 12px; color: #1a365d; font-weight: 900; font-size: 15px;">${formatMoney(investorCapitalPool)}</td>
                                        </tr>
                                        <tr style="border-bottom: 1px solid #e2e8f0;">
                                            <td style="padding: 12px; background-color: white; color: #475569;">تكلفة البناء والخدمات الإجمالية (ممول من المبيعات)</td>
        ...
        <div className="animate-fadeIn pb-12 font-cairo" dir="rtl">
            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 mb-8 flex flex-col lg:flex-row items-center gap-6">
                ...
                <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-[11px] font-black text-slate-500 mb-2">استدعاء دراسة محفوظة</label><select onChange={(e) => handleLoadCloud(e.target.value)} value={currentProjectId} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-2xl font-black text-[#1a365d] outline-none cursor-pointer focus:border-[#c5a059] transition-all"><option value="">-- اختر مشروعاً للقراءة --</option>{savedProjects.map(p => <option key={p.id} value={p.id}>{p.project_name}</option>)}</select></div>
                </div>
                <button onClick={handleSaveCloud} disabled={loading} className="w-full lg:w-auto bg-[#1a365d] text-white px-8 py-4 rounded-2xl font-black hover:bg-blue-900 transition-all flex items-center justify-center gap-3 shadow-lg shrink-0">{loading ? <RefreshCw size={20} className="animate-spin"/> : <Save size={20}/>} <span>حفظ المشروع</span></button>
            </div>
                ...
                <div className="xl:col-span-8 space-y-8">
                    ...
                    <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                        ...
                        <div className="p-6 md:p-8 space-y-8 bg-white">
                            ...
                            <div className="bg-slate-50 p-6 md:p-8 rounded-[2rem] border border-slate-100"><h3 className="text-sm font-black text-[#1a365d] mb-6 flex items-center gap-2"><Briefcase size={18} className="text-[#c5a059]" /> المصاريف الإدارية والتأسيس والرخص</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-6"><InputField label="رخصة وافي" name="sWafi" value={inputs.sWafi} onChange={handleChange} /><InputField label="المكتب الهندسي" name="sEng" value={inputs.sEng} onChange={handleChange} /><InputField label="رخصة البلدية" name="sMunicipality" value={inputs.sMunicipality} onChange={handleChange} /><InputField label="مكتب مشرف" name="sSupervision" value={inputs.sSupervision} onChange={handleChange} /><InputField label="محاسب قانوني" name="sAcc" value={inputs.sAcc} onChange={handleChange} /><InputField label="مصاريف أخرى" name="sOther" value={inputs.sOther} onChange={handleChange} /><InputField label="تأمين (% من بناء)" name="sInsurancePct" value={inputs.sInsurancePct} onChange={handleChange} step="0.1" /><InputField label="فحص (% من بناء)" name="sTestingPct" value={inputs.sTestingPct} onChange={handleChange} step="0.1" /></div></div>
                    ...
                    <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                        ...
                        <div className="overflow-x-auto p-6 md:p-8 bg-white"><table className="w-full text-right text-sm border-separate border-spacing-y-3"><thead><tr className="text-slate-400 font-black text-[11px] uppercase tracking-wider"><th className="px-4 pb-2">اسم المستثمر</th><th className="px-4 pb-2">المبلغ المستثمر (SAR)</th><th className="px-4 pb-2 text-center">نسبة الإسهام</th><th className="px-4 pb-2 text-center">الربح المتوقع</th><th className="px-4 pb-2 text-center">إجراء</th></tr></thead><tbody>{investors.map((inv, i) => {const pct = investorCapitalPool > 0 ? (inv.amount / investorCapitalPool) * 100 : 0; const prof = invProfitPool * (pct / 100); return (<tr key={i} className="bg-slate-50 transition-all rounded-2xl group"><td className="p-2 rounded-r-2xl"><input type="text" value={inv.name} onChange={e=>handleInvestorChange(i, 'name', e.target.value)} className="w-full bg-white border border-slate-200 px-4 py-3 rounded-xl font-bold text-[#1a365d] outline-none focus:border-[#c5a059]" placeholder="اسم المستثمر..." /></td><td className="p-2"><input type="number" value={inv.amount} onChange={e=>handleInvestorChange(i, 'amount', e.target.value)} className="w-full bg-white border border-slate-200 px-4 py-3 rounded-xl font-black text-[#1a365d] outline-none focus:border-[#c5a059]" /></td><td className="p-2 text-center font-black text-slate-500">{pct.toFixed(1)}%</td><td className="p-2 text-center font-black text-emerald-600">{formatMoney(prof)}</td><td className="p-2 text-center rounded-l-2xl"><button onClick={() => removeInvestor(i)} className="text-slate-400 hover:text-red-500 transition-colors p-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-red-100"><Trash2 size={18}/></button></td></tr>)})}</tbody><tfoot><tr><td className="p-4 font-black text-[#1a365d]">الإجمالي المجمع</td><td className="p-4 font-black text-[#1a365d] text-lg">{formatMoney(totalInvestedVal)}</td><td className="p-4 text-center font-black text-slate-400">{totalInvestedPct.toFixed(1)}%</td><td className="p-4 text-center font-black text-emerald-600 text-lg">{formatMoney(totalInvestedProfit)}</td><td className="p-4 text-center text-[10px] font-bold text-slate-400">تأكد من المطابقة</td></tr></tfoot></table></div>
                    </div>
                </div>

                <div className="xl:col-span-4 space-y-8">
                    <div className="bg-[#1a365d] text-white rounded-[2rem] shadow-2xl p-8 border-b-[12px] border-[#c5a059] relative overflow-hidden"><div className="absolute -left-6 -top-6 opacity-[0.03] transform -rotate-12"><Calculator size={200}/></div><h3 className="text-lg font-black text-[#c5a059] mb-8 flex items-center gap-2 relative z-10"><FileText size={20}/> ملخص اقتصاديات المشروع</h3><div className="grid grid-cols-2 gap-4 mb-8 relative z-10"><div className="bg-white/5 p-4 rounded-2xl text-center border border-white/10 backdrop-blur-sm"><span className="block text-[10px] font-bold text-slate-400 mb-1 uppercase tracking-widest">تكلفة أرض للمتر</span><span className="text-lg font-black text-white">{formatMoney(landCostPerSqm)}</span></div><div className="bg-white/5 p-4 rounded-2xl text-center border border-white/10 backdrop-blur-sm"><span className="block text-[10px] font-bold text-slate-400 mb-1 uppercase tracking-widest">تكلفة بناء للمتر</span><span className="text-lg font-black text-red-300">{formatMoney(totalCostPerSqm)}</span></div></div><div className="space-y-4 text-sm relative z-10"><div className="flex justify-between items-center border-b border-white/10 pb-3"><span className="font-bold text-slate-300">إجمالي المبيعات</span> <span className="text-emerald-400 font-black text-lg">{formatMoney(totalSales)}</span></div><div className="flex justify-between items-center border-b border-white/10 pb-3"><span className="font-bold text-slate-300">تكلفة البناء</span> <span className="text-red-300 font-black">{formatMoney(buildCost)}</span></div><div className="flex justify-between items-center border-b border-white/10 pb-3"><span className="font-bold text-slate-300">التأسيس والرخص</span> <span className="text-red-300 font-black">{formatMoney(softCosts)}</span></div><div className="flex justify-between items-center border-b border-white/10 pb-3"><span className="font-bold text-slate-300">السعي والتسويق</span> <span className="text-orange-400 font-black">{formatMoney(marketingCost)}</span></div><div className="flex justify-between items-center pt-2"><span className="font-bold text-white">رأس المال المستهدف</span> <span className="text-white font-black text-lg">{formatMoney(investorCapitalPool)}</span></div><div className="flex justify-between items-center pt-6 mt-4 border-t border-white/20"><span className="font-black text-white text-base">صافي الربح الكلي</span> <span className="text-emerald-400 font-black text-2xl">{formatMoney(netProfit)}</span></div></div><div className="grid grid-cols-2 gap-4 mt-8 relative z-10"><div className="bg-[#c5a059]/10 p-4 rounded-2xl text-center border border-[#c5a059]/30"><span className="block text-[10px] font-bold text-[#c5a059] mb-1">إجمالي ربح المستثمرين</span><span className="text-xl font-black text-white">{formatMoney(invProfitPool)}</span></div><div className="bg-white/10 p-4 rounded-2xl text-center border border-white/20"><span className="block text-[10px] font-bold text-slate-300 mb-1">صافي ربح المطور</span><span className="text-xl font-black text-white">{formatMoney(devProfit)}</span></div></div></div>
                    ...
                    <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="block text-[11px] font-black text-slate-500 mb-3 uppercase tracking-wider">طريقة عرض المستثمرين</label>
                                <div className="relative">
                                    <select value={printMode} onChange={e=>setPrintMode(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-4 py-3.5 rounded-2xl text-sm font-black text-[#1a365d] outline-none focus:border-[#c5a059] appearance-none cursor-pointer relative z-10">
                                        <option value="all">إظهار كافة المستثمرين</option><option value="summary">إظهار الإجمالي فقط</option><option value="single">تخصيص لمستثمر محدد</option>
                                    </select>
                            ...
                            <div className="flex flex-col gap-4 border-t border-slate-100 pt-6">
                                <button onClick={() => exportToPDF('teaser')} className="w-full bg-[#c5a059] text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-yellow-600 transition-all shadow-lg text-base"><Presentation size={22}/> تصدير العرض الاستثماري</button>
                                <button onClick={() => exportToPDF('detailed')} className="w-full bg-[#1a365d] text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-blue-900 transition-all shadow-lg text-base"><FileSpreadsheet size={22}/> تصدير الملحق المالي (PDF)</button>
                            </div>
                        </div>
                    </div>
                </div>
