...
export default function FeasibilityCalc({ showToast }) {
    ...
    return (
        <div className="animate-fadeIn pb-10 font-cairo" dir="rtl">
            ...
            <div className="bg-white dark:bg-brand-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-brand-700 mb-8 flex flex-col md:flex-row items-end gap-4">
                <div className="flex-1 w-full">
                    <label className="block text-xs font-bold text-brand-800 dark:text-brand-300 mb-2">اسم المشروع الحالي</label>
                    <input type="text" value={projectName} onChange={e=>setProjectName(e.target.value)} placeholder="اكتب اسم المشروع للحفظ..." className="w-full bg-slate-50 dark:bg-brand-900 border border-slate-200 dark:border-brand-700 dark:text-brand-50 dark:placeholder-brand-500 p-3 rounded-xl font-bold text-brand-800 outline-none focus:border-gold-500" />
                </div>
                ...
                <div className="w-px bg-slate-200 dark:bg-brand-700 hidden md:block h-12 mx-2"></div>
                <div className="flex-1 w-full flex items-end gap-2">
                    <div className="w-full">
                        <label className="block text-xs font-bold text-slate-500 dark:text-brand-400 mb-2">المشاريع المحفوظة</label>
                        <select onChange={(e) => handleLoadCloud(e.target.value)} value={currentProjectId} className="w-full bg-slate-50 dark:bg-brand-900 border border-slate-200 dark:border-brand-700 dark:text-brand-50 p-3 rounded-xl font-bold text-brand-800 outline-none cursor-pointer">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                ...
                <div className="xl:col-span-8 space-y-8">
                    {/* 1. المعماري */}
                    <div className="bg-white dark:bg-brand-900 rounded-3xl shadow-md border border-slate-200 dark:border-brand-700 overflow-hidden">
                        <div className="bg-indigo-900 p-4 text-white flex items-center gap-3">
                            <Calculator className="text-gold-500" /> <h2 className="text-lg font-black">1. الموجه المعماري والمالي الأساسي</h2>
                        </div>
                        <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 dark:bg-brand-800/40 border-b border-slate-100 dark:border-brand-700">
                            <div><label className="text-[10px] font-bold block mb-1 dark:text-brand-300">مساحة الأرض</label><input type="number" name="archLandArea" value={inputs.archLandArea} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50" /></div>
                            <div><label className="text-[10px] font-bold block mb-1 dark:text-brand-300">خدمات الدور</label><input type="number" name="archCommonArea" value={inputs.archCommonArea} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50" /></div>
                            <div><label className="text-[10px] font-bold block mb-1 dark:text-brand-300">الأدوار المتكررة</label><input type="number" name="archFloorsCount" value={inputs.archFloorsCount} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50" /></div>
                            <div><label className="text-[10px] font-bold block mb-1 dark:text-brand-300">نسبة الملحق %</label><input type="number" name="archRoofPct" value={inputs.archRoofPct} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50" /></div>
                        </div>
                        <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-4">
                            <div className="border dark:border-brand-700 p-3 rounded-xl dark:bg-brand-900"><label className="text-xs text-slate-500 dark:text-brand-400 block">بناء الأرضي %</label><input type="number" name="archGroundPct" value={inputs.archGroundPct} onChange={handleChange} className="w-full border-b dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 mb-2" /><label className="text-xs text-gold-500 font-bold block">وحدات الأرضي</label><input type="number" name="uGround" value={inputs.uGround} onChange={handleChange} className="w-full font-bold dark:bg-brand-900 dark:text-brand-50" /></div>
                            <div className="border dark:border-brand-700 p-3 rounded-xl dark:bg-brand-900"><label className="text-xs text-slate-500 dark:text-brand-400 block">بناء المتكرر %</label><input type="number" name="archTypicalPct" value={inputs.archTypicalPct} onChange={handleChange} className="w-full border-b dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 mb-2" /><label className="text-xs text-gold-500 font-bold block">وحدات المتكرر</label><input type="number" name="uTypical" value={inputs.uTypical} onChange={handleChange} className="w-full font-bold dark:bg-brand-900 dark:text-brand-50" /></div>
                            <div className="border dark:border-brand-700 p-3 rounded-xl dark:bg-brand-900 flex flex-col justify-end"><label className="text-xs text-gold-500 font-bold block mb-2">وحدات الملحق</label><input type="number" name="uRoof" value={inputs.uRoof} onChange={handleChange} className="w-full font-bold dark:bg-brand-900 dark:text-brand-50" /></div>
                        </div>
                    </div>

                    {/* 2. المالي */}
                    <div className="bg-white dark:bg-brand-900 rounded-3xl shadow-md border border-slate-200 dark:border-brand-700 p-6">
                        <h2 className="text-xl font-black text-brand-800 dark:text-brand-100 mb-4 border-b dark:border-brand-700 pb-4">2. التكاليف والرخص والمحاصة</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div className="bg-slate-50 dark:bg-brand-800/40 p-4 rounded-xl border dark:border-brand-700 flex gap-2">
                                <div className="flex-1"><label className="text-[10px] font-bold dark:text-brand-300">سعر البيع</label><input type="number" name="finSellPrice" value={inputs.finSellPrice} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50" /></div>
                                <div className="flex-1"><label className="text-[10px] font-bold text-orange-600 dark:text-orange-300">تكلفة البناء</label><input type="number" name="finBuildCost" value={inputs.finBuildCost} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 text-orange-600 dark:text-orange-300" /></div>
                                <div className="flex-1"><label className="text-[10px] font-bold dark:text-brand-300">قيمة الأرض</label><input type="number" name="finLandPrice" value={inputs.finLandPrice} onChange={handleChange} className="w-full p-2 rounded border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50" /></div>
                            </div>
                            <div className="bg-blue-50 dark:bg-blue-500/10 p-4 rounded-xl border border-blue-100 dark:border-blue-500/30 grid grid-cols-3 gap-2">
                                <div><label className="text-[9px] dark:text-brand-400">وافي</label><input type="number" name="sWafi" value={inputs.sWafi} onChange={handleChange} className="w-full p-1 border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50 text-xs rounded"/></div>
                                <div><label className="text-[9px] dark:text-brand-400">هندسي</label><input type="number" name="sEng" value={inputs.sEng} onChange={handleChange} className="w-full p-1 border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50 text-xs rounded"/></div>
                                <div><label className="text-[9px] dark:text-brand-400">البلدية</label><input type="number" name="sMunicipality" value={inputs.sMunicipality} onChange={handleChange} className="w-full p-1 border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50 text-xs rounded"/></div>
                                <div><label className="text-[9px] dark:text-brand-400">مشرف</label><input type="number" name="sSupervision" value={inputs.sSupervision} onChange={handleChange} className="w-full p-1 border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50 text-xs rounded"/></div>
                                <div><label className="text-[9px] dark:text-brand-400">محاسب</label><input type="number" name="sAcc" value={inputs.sAcc} onChange={handleChange} className="w-full p-1 border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50 text-xs rounded"/></div>
                                <div><label className="text-[9px] dark:text-brand-400">أخرى</label><input type="number" name="sOther" value={inputs.sOther} onChange={handleChange} className="w-full p-1 border dark:bg-brand-900 dark:border-brand-700 dark:text-brand-50 text-xs rounded"/></div>
                            </div>
                        </div>
                        <div className="bg-orange-50 dark:bg-amber-500/10 p-4 rounded-2xl flex items-center gap-4 border border-orange-100 dark:border-amber-500/30">
                            <div className="flex-1 flex gap-2">
                                <div className="flex-1"><label className="text-[10px] font-bold text-orange-800 dark:text-amber-300 mb-1 block">علاوة مستثمر %</label><input type="number" name="inInvBonusPct" value={inputs.inInvBonusPct} onChange={handleChange} className="w-full p-2 rounded border border-orange-200 dark:border-brand-700 dark:bg-brand-900 text-orange-700 dark:text-amber-300 font-bold" /></div>
                                <div className="flex-1"><label className="text-[10px] font-bold text-emerald-800 dark:text-emerald-300 mb-1 block">تسويق وسعي %</label><input type="number" name="sMarkPct" value={inputs.sMarkPct} onChange={handleChange} className="w-full p-2 rounded border border-emerald-200 dark:border-brand-700 dark:bg-brand-900 text-emerald-700 dark:text-emerald-300 font-bold" /></div>
                                <div className="flex-1"><label className="text-[10px] font-bold text-slate-800 dark:text-brand-300 mb-1 block">مدة المشروع</label><input type="number" name="sDuration" value={inputs.sDuration} onChange={handleChange} className="w-full p-2 rounded border border-slate-200 dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 font-bold" /></div>
                            </div>
                            <div className="w-32 text-center bg-white dark:bg-brand-900 p-2 rounded-xl border dark:border-brand-700 shadow-sm"><span className="block text-[9px] text-slate-400 dark:text-brand-400">حصة المستثمر</span><span className="text-xl font-black text-emerald-600 dark:text-emerald-300">{finalInvPct.toFixed(1)}%</span></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-brand-900 rounded-3xl shadow-md border border-slate-200 dark:border-brand-700 p-6">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-black text-brand-800 dark:text-brand-100"><Users className="inline text-gold-500 mr-2"/>قائمة المستثمرين</h2>
                            <button onClick={addInvestorRow} className="bg-brand-800 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-800 transition flex items-center gap-1 shadow-sm"><Plus size={14}/> إضافة مستثمر</button>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-right text-sm">
                                <thead className="bg-slate-50 dark:bg-brand-800/60 border-b border-slate-100 dark:border-brand-700">
                                    <tr className="text-slate-600 dark:text-brand-300">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-brand-700">
                                    {investors.map((inv, i) => {
                                        ...
                                        const prof = invProfitPool * (pct / 100);
                                        return (
                                            <tr key={i} className="hover:bg-slate-50 dark:hover:bg-brand-800 transition">
                                                <td className="p-2"><input type="text" value={inv.name} onChange={e=>handleInvestorChange(i, 'name', e.target.value)} className="border border-slate-200 dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 dark:placeholder-brand-500 p-2 rounded-lg w-full text-sm outline-none focus:border-gold-500" placeholder="اسم المستثمر..." /></td>
                                                <td className="p-2"><input type="number" value={inv.amount} onChange={e=>handleInvestorChange(i, 'amount', e.target.value)} className="border border-slate-200 dark:border-brand-700 dark:bg-brand-900 p-2 rounded-lg w-full text-sm font-black text-brand-800 dark:text-brand-50 outline-none focus:border-gold-500" /></td>
                                                <td className="p-2 text-emerald-600 dark:text-emerald-300 font-black text-center bg-slate-50/50 dark:bg-brand-800/20">{formatMoney(prof)}</td>
                                                <td className="p-2 text-center"><button onClick={() => removeInvestor(i)} className="text-red-400 hover:text-red-600 bg-white dark:bg-brand-900 border border-slate-200 dark:border-brand-700 p-1.5 rounded shadow-sm transition"><Trash2 size={16}/></button></td>
                                            </tr>
                                        )
                                    })}
                                </tbody>
                                <tfoot className="bg-slate-50 dark:bg-brand-800/60 font-black border-t-2 border-slate-200 dark:border-brand-700">
                                    <tr>
                                        <td className="p-3 text-brand-800 dark:text-brand-100">الإجمالي المجمع</td>
                                        <td className="p-3 text-brand-800 dark:text-brand-100">{formatMoney(totalInvestedVal)}</td>
                                        <td className="p-3 text-center text-emerald-600 dark:text-emerald-300">{formatMoney(invProfitPool * (totalInvestedPct/100))}</td>
                                        <td className="p-3 text-center text-xs text-slate-500 dark:text-brand-400">{totalInvestedPct.toFixed(1)}%</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                <div className="xl:col-span-4 space-y-6">
                    ...
                    <div className="bg-white dark:bg-brand-900 p-6 rounded-[2rem] shadow-md border border-slate-200 dark:border-brand-700">
                        <label className="block text-sm font-black text-brand-800 dark:text-brand-100 mb-4 border-b border-slate-100 dark:border-brand-700 pb-3">خيارات العروض والطباعة</label>

                        <div className="mb-4">
                            <label className="block text-[11px] font-bold text-slate-500 dark:text-brand-400 mb-2">طريقة عرض المستثمرين في التقرير:</label>
                            <select value={printMode} onChange={e=>setPrintMode(e.target.value)} className="w-full p-2.5 border border-slate-200 dark:border-brand-700 dark:bg-brand-900 dark:text-brand-50 rounded-lg mb-2 text-sm font-bold bg-slate-50 outline-none focus:border-gold-500">
                            ...
                            {printMode === 'single' && (
                                <select value={selectedInvestorIndex} onChange={e=>setSelectedInvestorIndex(e.target.value)} className="w-full p-2.5 border border-gold-500 dark:border-brand-700 rounded-lg mb-2 text-sm font-black bg-orange-50 dark:bg-amber-500/10 text-orange-800 dark:text-amber-300 outline-none">
                                    {investors.map((inv, i) => <option key={i} value={i}>{inv.name || `مستثمر ${i+1}`}</option>)}
                                </select>
                            )}
                        </div>

                        <div className="flex items-center gap-3 bg-slate-50 dark:bg-brand-800/40 p-3 rounded-xl border border-slate-100 dark:border-brand-700 mb-5 cursor-pointer">
                            <input type="checkbox" id="devToggle" checked={showDevInPrint} onChange={e=>setShowDevInPrint(e.target.checked)} className="w-4 h-4 accent-gold-500 cursor-pointer" />
                            <label htmlFor="devToggle" className="text-xs font-bold text-brand-800 dark:text-brand-300 cursor-pointer select-none">إظهار تفاصيل المطور في الملحق</label>
                        </div>

                        <div className="flex flex-col gap-3 border-t border-slate-100 dark:border-brand-700 pt-5">
                            <button onClick={handlePrintTeaser} className="w-full bg-gold-500 text-white py-3.5 rounded-xl font-black flex items-center justify-center gap-2 hover:bg-yellow-600 transition shadow-lg"><Presentation size={18}/> العرض الاستثماري (الملخص)</button>
                            <button onClick={handlePrintDetailed} className="w-full bg-brand-800 text-white py-3.5 rounded-xl font-black flex items-center justify-center gap-2 hover:bg-blue-900 transition shadow-lg"><FileSpreadsheet size={18}/> الملحق المالي التفصيلي</button>
                        </div>
                    </div>
                </div>
            </div>
