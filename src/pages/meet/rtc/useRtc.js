...
export default function useRtc({ guestToken = null, meetingId = 0 } = {}) {
    ...
    const headers = useCallback(() => (guestToken
        ? { 'Content-Type': 'application/json', 'X-Guest-Token': guestToken }
        : { 'Content-Type': 'application/json', Authorization: 'Bearer ' + getAdminToken() }), [guestToken]);

    const post = useCallback(async (action, body, keepalive) => {
        const res = await fetch(`${API_URL}?action=${action}`, {
            method: 'POST', headers: headers(), body: JSON.stringify(body || {}), keepalive: !!keepalive,
        });
        return res.json();
    }, [headers]);
