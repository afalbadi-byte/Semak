// شبكة إنستقرام ثلاثية متصلة (بانوراما ٣٢٤٠×١٤٤٠ تُقسم إلى ثلاث منشورات ٣:٤) بألوان الطباع الستة:
...
// الاستعمال: node build-grid.cjs [طبع|all] → grid-<طبع>-1-left.png · -2-center.png · -3-right.png (٢١٦٠×٢٨٨٠) + grid-<طبع>-preview.jpg
...
// لكل طبع: الأرضية ولون النص ولون التمييز، ونقش الشريط كما في منشورات الطباع
const TRAITS = {
  vision:        { color: '#7c5d21', ink: '#7c5d21', accent: '#5aba1c', text: '#ffffff', band: 'vision-p3.png' },
  determination: { color: '#971a4d', ink: '#971a4d', accent: '#5aba1c', text: '#ffffff', band: 'determination-p1.png' },
  courage:       { color: '#607c4f', ink: '#4c6640', accent: '#b8ec84', text: '#ffffff', band: 'courage-p3.png' },
  authenticity:  { color: '#5aba1c', ink: '#002628', accent: '#002628', text: '#002628', band: 'authenticity-p1.png' },
  generosity:    { color: '#0050af', ink: '#0050af', accent: '#5aba1c', text: '#ffffff', band: 'generosity-p3.png' },
  giving:        { color: '#6565e0', ink: '#4f4fc4', accent: '#5aba1c', text: '#ffffff', band: 'giving-p2.png' },
};
...
.band{position:absolute;left:900px;width:1440px;top:330px;height:500px;background:url('${b64(T.band)}') center/500px 500px repeat-x;
...
.semak{position:absolute;top:1080px;left:${1620 - 140}px;width:280px;filter:${T.text === '#ffffff' ? 'brightness(0) invert(1)' : 'brightness(0)'}}
...
.stat{border:2px solid ${T.text}59;border-radius:26px;padding:18px 26px 20px;background:rgba(0,0,0,.08)}
...
/* رمز البروشور بارتفاع عمود التواصل (١١٠٠→١٣٩٠) */
.qr{position:absolute;right:60px;top:1100px;width:260px;text-align:center}
.qr .c{display:inline-block;background:#fff;border-radius:20px;padding:12px;box-shadow:0 10px 26px rgba(0,0,0,.25)}
.qr img{display:block;width:222px;height:222px}
.qr span{display:block;margin-top:10px;font-family:'Plex';font-weight:700;font-size:26px;white-space:nowrap}
.rule{position:absolute;left:2380px;right:80px;top:1072px;height:2px;background:${T.text};opacity:.3}
${K.css}
.k-row span{color:${T.text}} .k-row svg{stroke:${T.text}}
...
<img class="frame" src="${b64(`${k}-frame.png`)}">
...
(async () => {
  ...
  for (const [k, T] of Object.entries(TRAITS)) {
    ...
    const tiles = [];
    for (let i = 0; i < 3; i++) {
      await sharp(buf).extract({ left: i * 2160, top: 0, width: 2160, height: 2880 }).png().toFile(path.join(D, `grid-${k}-${names[i]}.png`));
